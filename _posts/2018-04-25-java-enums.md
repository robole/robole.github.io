...
We usually refer to a value by _enum.value_ e.g. `CoffeeSize.SMALL`.
...
Let's get more specific about our sizes, and add a variable for the volume, measured in (fluid) ounces. We can call it _ounces_. We want to set the value of _ounces_ for each size on creation. How do we do that? We will do this through the constructor.
