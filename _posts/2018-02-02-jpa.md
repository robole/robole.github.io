...
- If you want to annotate variables and a getter method, it changes the access type of the entity to property access. Then, you need to be explicit with the access for everything. Below, we say that the access type is on fields for everything, except getId().
...
|Name|Target|Purpose|
|---------------|----------------|------------------|
|    Entity     |    continue    |   Declare the class as entity. |
|Table| | Assign a table for the entity. The JPA default table name is the name of the class (minus the package) with the first letter capitalized.      Can override name @Table(name="EMPLOYEE")|
...
|    Column     |    continue    | Can provide the name of the column in a table if you want it to be different from the attribute name. By default, the two names are assumed to be the same. |
|    id     |    continue    |   Signifies the primary key. |
|    GeneratedValue     |    continue    |   ignifies a strategy to assign a unique value to your identity fields automatically. The types of strategies available are IDENTITY, SEQUENCE, TABLE, and AUTO. The default strategy is auto. |
...
|@NamedQuery|	|This annotation is used for specifying a Query using static  |
